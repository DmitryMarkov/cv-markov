...
- `yarn build:images` — `node scripts/generate-image-variants.mjs`. Manual, offline regeneration of WebP variants for `assets/images/avatar-dm.png` and `avatar-dm-2x.png` (sharp at quality 85). Output sits alongside the PNGs and is committed; `<picture>` in `src/index.html` uses WebP as primary source with PNG fallback. Re-run only when the source PNGs change.
...
- `vite-plugin-static-copy` copies `assets/` → `build/assets/` and `src/robots.txt` → `build/robots.txt` and serves the same files at the matching dev-server paths. Source is an absolute path against `import.meta.dirname` because the plugin's fast-glob doesn't accept Vite's `root: 'src'`-relative paths well.
- `publicDir` is `false` — there is no Vite public folder; everything served at the site root comes from the static-copy plugin or build outputs. Favicons live under `/assets/favicons/` (see "Favicons" below); `robots.txt` sits at the site root because `yarn preview`'s SPA fallback otherwise serves `index.html` for `/robots.txt`, which Lighthouse parses as invalid robots and drops SEO to 92.
...
The page content is wrapped in `<main>` so the `landmark-one-main` audit passes; `<header>` stays outside `<main>` per the standard pattern.
...
- `sharp` is used directly by `scripts/generate-image-variants.mjs` but is intentionally **not** declared in `package.json`. The `favicons` package depends on it via `^0.33.1`, and for major-zero semver that range matches `0.33.x` only. Declaring sharp here at any other version would land a duplicate install in `node_modules` (sharp is a ~70 MB native addon). The trade-off: when favicons widens its sharp constraint (or jumps to a newer sharp major), Renovate will bump the chain transitively and our script will get the new sharp for free. Until then we deliberately share favicons' resolution. If `yarn build:images` ever errors out with `Cannot find module 'sharp'`, that's the signal — at that point declare sharp directly with the version favicons currently pulls.
